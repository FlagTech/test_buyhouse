...
def buy_or_rent(total_price, rent_payment, income, consume, invest_rate):
    ...
    data['total_asset_loan'] = (data['total_asset_loan'] + data['house_price'] -
                                data['principal_left'])
    data['rent_investment'] = data['income'] - data['consume'] - data['rent_payment']
    data['total_asset_rent'] = investment_fun(data['rent_investment'], invest_rate_f=invest_rate,
                                              first_payment=(total_price * 0.2)) 

    # 折線圖
    fig_line = make_subplots(specs=[[{"secondary_y": True}]])
    fig_line.add_trace(
        go.Scatter(x=data['date'], y=data['total_asset_loan'], fill='tozeroy',
                   name='買房者資產'),
        secondary_y=False,)

    fig_line.add_trace(
        go.Scatter(x=data['date'], y=data['total_asset_rent'], fill='tozeroy',
                   name='租房者資產'),
        secondary_y=False,)
    ...
    fig_line.update_layout(
        paper_bgcolor='rgba(0,0,0,0)', height=500, width=1000,
        legend=dict(
            x=0, y=1,  # 圖例位置
            traceorder="normal",
            font=dict(
    ...
    fig.update_layout(
        annotations=[
            dict(text='購屋', x=0.18, y=0.5, font_size=20, showarrow=False),
