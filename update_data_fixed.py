...
def process_quarter_data():
    ...
    try:
        ...
        # 移除重複的新增資料，只保留到111Q1
        existing_data = existing_data[existing_data['季度'] <= '111Q1']
        print(f"保留原始資料到: {existing_data.iloc[-1, 0]}")
    except:
    ...
    for quarter_dir in new_quarters:
        ...
        if not all_rent_data.empty:
            # 資料清洗 - 租金 (注意租金資料的標的名稱不同)
            filter_row = (all_rent_data['交易標的'] == '房地(土地+建物)') | \
                         (all_rent_data['交易標的'] == '房地(土地+建物)+車位') | \
                         (all_rent_data['交易標的'] == '租賃房屋') | \
                         (all_rent_data['交易標的'] == '租賃房屋+車位')
        ...
        for key, city_name in location.items():
            ...
            if not city_rent_data.empty:
                # 租金資料的交易標的處理
                filter_row = (city_rent_data['交易標的'] == '房地(土地+建物)') | \
                             (city_rent_data['交易標的'] == '房地(土地+建物)+車位') | \
                             (city_rent_data['交易標的'] == '租賃房屋') | \
                             (city_rent_data['交易標的'] == '租賃房屋+車位')
...
def update_other_files(updated_data):
    ...
    # 更新rent.csv，計算時排除0值
    rent_data_with_zeros = updated_data[['季度', '租金(元/坪)']].dropna()
    ...
    for idx, row in updated_data.iterrows():
        if pd.isna(row['租金(元/坪)']):
            # 重新計算該季度的平均租金，排除0值
            city_rents = [row[col] for col in rent_columns if col in row and row[col] > 0]
            if city_rents:
                avg_rent = int(sum(city_rents) / len(city_rents))
                recalculated_rents.append(avg_rent)
            else:
                recalculated_rents.append(np.nan)
        else:
            recalculated_rents.append(row['租金(元/坪)'])
    
    # 更新整體租金資料
    updated_data['租金(元/坪)'] = recalculated_rents
